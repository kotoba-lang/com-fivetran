(ns fivetran.main
  "Kotodama WASM entrypoint for the Fivetran clean-room actor (L4) — Clojure port.
  ...
  facts (`fivetran.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fivetran")
(def tier "L4")
...
(def entity-specs
  [{:entity "Connector"      :plural "connectors"      :id-prefix "fivetran_con"
    :fields [:service :schema :groupId :setupState :syncState :updateState :scheduleType :networkingMethod :syncFrequency :paused :createdAt :succeededAt :failedAt] :required [:service]
    :coerce {:syncFrequency :int :paused :bool} :refs {:groupId "Group"}}
   {:entity "Destination"    :plural "destinations"    :id-prefix "fivetran_des"
    :fields [:groupId :service :region :setupStatus :networkingMethod :daylightSavingTimeEnabled :createdAt] :required [:service]
    :coerce {:daylightSavingTimeEnabled :bool} :refs {:groupId "Group"}}
   {:entity "Group"          :plural "groups"          :id-prefix "fivetran_gro"
    :fields [:name :createdAt]                         :required [:name]
    :coerce {}                       :refs {}}
   {:entity "User"           :plural "users"           :id-prefix "fivetran_use"
    :fields [:email :givenName :familyName :role :verified :active :createdAt] :required [:email]
    :coerce {:verified :bool :active :bool}           :refs {}}
   {:entity "Transformation" :plural "transformations" :id-prefix "fivetran_tra"
    :fields [:type :status :paused :createdAt :lastStartedAt :lastEndedAt] :required [:type]
    :coerce {:paused :bool}          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"fivetran.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fivetran-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fivetran-compat/ready)
